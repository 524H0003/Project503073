...
export default function Dashboard() {
  ...
  return Layout(
    user ? (
      <div className="flex h-full flex-col bg-[radial-gradient(circle_at_top_left,_#e0e7ff,_white,_#f8fafc)]">
        {/* Header */}
        <div className="sticky top-0 z-20 border-b border-white/50 bg-white/75 px-4 py-4 shadow-sm backdrop-blur-xl sm:px-6">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div className="flex items-center gap-4">
              <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-gradient-to-br from-indigo-400 to-cyan-300 shadow-md transition-transform duration-200 hover:scale-105">
        ...
        {/* Search */}
        <div className="sticky top-[92px] z-10 w-full px-4 pt-4 backdrop-blur-sm sm:px-6">
          <SearchBar />
        </div>
              ...
              {notes.map((note) => (
                ...
                >
                  <CardHeader className="p-4 pb-2">
                    <CardTitle className="line-clamp-1 break-words text-lg text-slate-700">
                      {note.title || "Ghi chú không tiêu đề"}
                    </CardTitle>
                  </CardHeader>

                  <CardContent className="p-4 pt-0">
                    <p className="line-clamp-3 break-words text-sm leading-relaxed text-slate-500/90">
